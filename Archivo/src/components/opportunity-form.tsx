...
export function OpportunityForm({ opportunity = null, onComplete }) {
  ...
  const {
    ...
    defaultValues: opportunity
      ? {
          ...
          assignAgentId: opportunity.assignAgent?.id?.toString() || "",
          ...
          properties: Array.isArray(opportunity.properties)
            ? opportunity.properties.map((p) => p?.id?.toString() || "").filter(Boolean)
            : [],
        }
      : {
          name: "",
  ...
  return (
    <Card sx={{ width: "100%", maxWidth: "100%", overflow: "visible" }}>
      <CardHeader
        ...
        sx={{
          bgcolor: getStageColor(currentStage),
        ...
        <TabPanel value={activeTab} index={2}>
          {opportunity && opportunity.id && (
            <OpportunityOffers
              ...
              leadId={opportunity.prospect?.id}
            />
          )}
        </TabPanel>
        ...
        >
          ...
          {opportunity && (
            <Button
              ...
              fullWidth={isMobile}
              onClick={() => {
                // Aquí iría la lógica para eliminar la oportunidad
                console.log("Eliminar oportunidad", opportunity.id)
                onComplete()
              }}
            >
